...
impl Game {
    ...
    fn handle_action(&mut self, action: Option<GameAction>) {
        let Some(action) = action else { return };
        let mut acted = false;
        let mut delta = None;
        match action {
            GameAction::Move(dx, dy) => delta = Some(Point::new(dx, dy)),
            GameAction::Wait => {
                acted = true;
            }
            GameAction::Descend => {
            ...
            GameAction::Pickup => acted = self.pickup(),
            GameAction::Quaff => acted = self.quaff(),
            GameAction::Eat => acted = self.eat(),
            GameAction::Read => acted = self.read_scroll(),
            GameAction::PutOnRing => acted = self.put_on_ring(),
            GameAction::RemoveRing => acted = self.remove_ring(),
            GameAction::Search => acted = self.search_for_secrets(),
            GameAction::Throw => acted = self.throw_item(),
            GameAction::Explore => acted = self.auto_explore_step(),
            _ => {}
        }
        if let Some(d) = delta {
    ...
    fn render(&mut self) {
        match self.mode {
            ...
            Mode::ConfirmQuit => {
                ...
                self.render_confirm_quit();
            }
            Mode::Dead => {
                self.render_play();
                self.render_banner("You have died. Enter: new game   S: scores   Esc: quit");
            }
            Mode::Won => self.render_won(),
            Mode::MessageLog => {
    ...
    fn render_confirm_quit(&mut self) {
    ...
    fn render_title(&mut self) {
        self.fb.print_centered(8,  self.theme.header(), self.theme.bg(), "R O G U E");
        self.fb.print_centered(10, self.theme.fg(),     self.theme.bg(), "a modern Rust reimplementation");
        self.fb.print_centered(13, self.theme.header(), self.theme.bg(), "Press Enter to begin");
        self.fb.print_centered(14, self.theme.fg(),     self.theme.bg(), "S: high scores");
        self.fb.print_centered(16, self.theme.fg(),     self.theme.bg(), "Move: hjkl / yubn / arrows   Wait: .   Descend: >");
        self.fb.print_centered(17, self.theme.fg(),     self.theme.bg(), "Pick up: g   Quaff: q   Eat: e   Read: r   Inventory: i   Quit: Esc");
        self.fb.print_centered(19, self.theme.dim_ui(), self.theme.bg(), "Press ? in game for help   ·   A = autopilot bot");
    }
    ...
    fn render_help(&mut self) {
        ...
        let tx = x0 + pad;
        self.fb.print(tx, y0 + 1, self.theme.header(), self.theme.bg(), "HELP");
    ...
    fn render_inventory(&mut self) {
        ...
        self.fb.draw_box(x0, y0, BOX_W, BOX_H, self.theme.fg(), self.theme.bg());
        self.fb.print(tx, y0 + 1, self.theme.header(), self.theme.bg(),
            "INVENTORY   (letter=use  up/dn=scroll  Esc=close)");

        if self.inventory.is_empty() {
            self.fb.print(tx, y0 + 3, self.theme.dim_ui(), self.theme.bg(), "Your pack is empty.");
        } else {
            ...
            for i in 0..visible {
                ...
                self.fb.print(tx, y0 + 3 + i as i32, fg, self.theme.bg(), &line);
            }

            if scroll > 0 {
                self.fb.print(x0 + BOX_W - 3, y0 + 3, self.theme.accent(), self.theme.bg(), "^");
            }
            if scroll < max_scroll {
                self.fb.print(x0 + BOX_W - 3, y0 + 2 + visible as i32, self.theme.accent(), self.theme.bg(), "v");
            }

            let count_str = format!("{}/{} items", visible.min(total - scroll), total);
            self.fb.print(tx, y0 + BOX_H - 5, self.theme.dim_ui(), self.theme.bg(), &count_str);
        }

        let ring_y = y0 + BOX_H - 4;
        self.fb.print(tx, ring_y, self.theme.header(), self.theme.bg(), "Equipped rings:");
        ...
        let right_str = self.right_ring.map(|k| format!("ring of {}", ring_name(k)))
            .unwrap_or_else(|| "(none)".to_string());
        self.fb.print(tx, ring_y + 1, self.theme.fg(), self.theme.bg(),
            format!("  Left: {:<24}  Right: {}", left_str, right_str));
        self.fb.print(tx, ring_y + 2, self.theme.dim_ui(), self.theme.bg(),
            "  p: put on ring   Shift+R: remove ring");
    }

    fn render_message_log(&mut self) {
        ...
        self.fb.draw_box(x0, y0, box_w, box_h, self.theme.fg(), self.theme.bg());
        self.fb.print(tx, y0 + 1, self.theme.header(), self.theme.bg(), "MESSAGE LOG");

        if self.log.is_empty() {
            self.fb.print(tx, y0 + 3, self.theme.dim_ui(), self.theme.bg(), "(no messages yet)");
        } else {
            ...
            for (i, msg) in msgs.iter().skip(start).take(PANEL_LINES).enumerate() {
                ...
                self.fb.print(tx, y0 + 3 + i as i32, self.theme.fg(), self.theme.bg(), display);
            }
        }
        self.fb.print(
            tx,
    ...
    fn render_scores(&mut self) {
        ...
        self.fb.draw_box(x0, y0, box_w, box_h, self.theme.fg(), self.theme.bg());
        self.fb.print(tx, y0 + 1, self.theme.header(), self.theme.bg(), "HIGH SCORES");

        if lines.is_empty() {
            self.fb.print(tx, y0 + 3, self.theme.dim_ui(), self.theme.bg(), "(no scores recorded yet)");
        } else {
            for (i, line) in lines.iter().enumerate() {
                self.fb.print(tx, y0 + 3 + i as i32, self.theme.fg(), self.theme.bg(), line);
            }
        }
        self.fb.print(
            tx,
    ...
    fn render_won(&mut self) {
        ...
        self.fb.draw_box(x0, y0, box_w, box_h, self.theme.header(), self.theme.bg());

        self.fb.print_centered(y0 + 1, self.theme.header(), self.theme.bg(),
            "*** CONGRATULATIONS! YOU WIN! ***");
        self.fb.print_centered(y0 + 2, self.theme.accent(), self.theme.bg(),
            "You escaped the dungeon with the Amulet of Yendor!");

        self.fb.print(tx, y0 + 4, self.theme.fg(), self.theme.bg(), "SCORE SUMMARY");
        self.fb.print(tx, y0 + 5, self.theme.fg(), self.theme.bg(),
            format!("  Dungeon depth reached : {}", self.depth));
        self.fb.print(tx, y0 + 6, self.theme.fg(), self.theme.bg(),
            format!("  Gold collected        : {}", self.gold));
        self.fb.print(tx, y0 + 7, self.theme.fg(), self.theme.bg(),
            format!("  Turns taken           : {}", self.turns));

        if let Ok(s) = self.world.get::<&Stats>(self.player) {
            self.fb.print(tx, y0 + 8, self.theme.fg(), self.theme.bg(),
                format!("  Character level      : {}", s.level));
            self.fb.print(tx, y0 + 9, self.theme.fg(), self.theme.bg(),
                format!("  Experience           : {}", s.xp_reward));
        }

        self.fb.print(tx, y0 + 11, self.theme.accent(), self.theme.bg(),
            "Your score has been recorded.");
        self.fb.print(tx, y0 + 12, self.theme.accent(), self.theme.bg(),
            "Press S to view the hall of fame.");
        self.fb.print(tx, y0 + 13, self.theme.dim_ui(), self.theme.bg(),
    ...
    fn render_play(&mut self) {
        ...
        if let Some(last) = self.log.last() {
            self.fb.print(0, MSG_ROW, self.theme.fg(), self.theme.bg(), last);
        }
        ...
        for y in 0..self.map.height {
            for x in 0..self.map.width {
                ...
                if self.theme.use_tiled() {
                    ...
                    self.fb.set_raw(x, y + MAP_TOP, self.theme.apply(color), self.theme.bg(), glyph_u16);
                } else {
                    ...
                    self.fb.set(x, y + MAP_TOP, self.theme.apply(color), self.theme.bg(), glyph);
                }
            }
        }
    ...
    fn render_status(&mut self) {
        ...
        self.fb.print(0, row, self.theme.fg(), self.theme.bg(), line);
    }
    ...
    fn render_hints(&mut self) {
        ...
        if let Some(ctx_hint) = self.context_hint() {
            self.fb.print(0, hint_row, self.theme.header(), self.theme.bg(), ctx_hint);
        }
    ...
    pub fn tick(&mut self, action: Option<GameAction>) -> &FrameBuffer {
        match self.mode {
            Mode::Title => {
                if action == Some(GameAction::Confirm) {
                    self.mode = Mode::Playing;
                } else if action == Some(GameAction::Scores) {
                    self.mode = Mode::Scores;
                } else if action == Some(GameAction::Quit) {
                    self.request_quit();
                }
            }
            Mode::Playing => match action {
                Some(GameAction::Quit) => self.request_quit(),
                Some(GameAction::Autopilot) => self.toggle_autopilot(),
                Some(GameAction::Help) if !self.autopilot => self.mode = Mode::Help,
                Some(GameAction::Inventory) if !self.autopilot => self.mode = Mode::Inventory,
                Some(GameAction::MessageLog) if !self.autopilot => {
                    self.log_scroll = 0;
            ...
            Mode::Inventory => {
                const INV_PAGE: usize = 16;
                match action {
                    Some(GameAction::Cancel) | Some(GameAction::Quit) | Some(GameAction::Inventory) => {
                        self.mode = Mode::Playing;
                    }
                    Some(GameAction::ScrollUp) => {
                        if self.inv_scroll > 0 {
                            self.inv_scroll -= 1;
                        }
                    }
                    Some(GameAction::ScrollDown) => {
                        let max = self.inventory.len().saturating_sub(INV_PAGE);
                        if self.inv_scroll < max {
                            self.inv_scroll += 1;
                        }
                    }
                    Some(GameAction::Char(c)) => {
                        if let Some(idx) = char_to_inv_idx(c, self.inv_scroll) {
                            if idx < self.inventory.len() {
                                self.mode = Mode::Playing;
            ...
            Mode::ConfirmQuit => match action {
                Some(GameAction::Confirm) => self.fb.wants_quit = true,
                Some(GameAction::Cancel) => {
            ...
            Mode::Dead | Mode::Won => {
                match action {
                    ...
                    Some(GameAction::Scores) => self.mode = Mode::Scores,
                    Some(GameAction::Quit) => self.request_quit(),
                    _ => {}
                }
            }
...
/// Map a char to an inventory index (a–z → 0–25, 0–9 → 26–35).
fn char_to_inv_idx(c: char, scroll: usize) -> Option<usize> {
    if c.is_ascii_lowercase() {
        Some(scroll + (c as u8 - b'a') as usize)
    } else if c.is_ascii_digit() {
        Some(scroll + 26 + (c as u8 - b'0') as usize)
    } else {
        None
    }
}
